...
show_games() {
	menu_lines="$(wc -l < "$3")"
	[ "$menu_lines" -gt 30 ] && menu_lines=40
	game=$(m -l "$menu_lines" -p "$1" < "$2")
	[ ! "$game" ] && { clear_cache; exit 1; }
	line_number="$(grep -n "$game" "$2" | cut -d : -f 2)"
...
m() { rofi -dmenu -theme /home/angel/.rofi-themes/runnerforepic.rasi -i "$@"; }
alert() { rofi -e "$@"; }

get_games_list && pkill rofi

mode="$(
	printf "Downloaded games\nAll games" | m -l 1 -p "What do you want to see?"
)"
